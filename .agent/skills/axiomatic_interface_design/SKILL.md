---
name: Axiomatic Interface Design (MDA-inspired)
description: >-
...
- **$\{Pre\}$ (事前条件)**: 呼び出し側が保証すべき条件。実装冒頭の `FB_ASSERT` に変換される。
- **$\{Post\}$ (事後条件)**: 実装側が保証すべき結果。単体テストの `EXPECT_*/ASSERT_*` に変換される。
- **$[Invariant]$ (不変条件)**: オブジェクトの全ライフサイクルで維持されるべき状態の憲法。

## 2. MDA変換ルール：PIM(WIT) から PSM(C++) への導出
...
1.  **境界値テスト (Pre-condition Boundary)**:
    - `{Pre}` を満たす最小/最大の値。
    - `{Pre}` をわずかに外れる値（`FB_ASSERT` がトリガーされることの確認）。
2.  **正当性テスト (Post-condition Fulfillment)**:
    - メソッド実行後、内部状態が `{Post}` と一致するかを検証。
3.  **不変条件テスト (Invariant Preservation)**:
