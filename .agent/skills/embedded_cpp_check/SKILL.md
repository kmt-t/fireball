...
- **Docker Workaround**: チェックツールの実行にはDockerコンテナを推奨します。詳細は [Docker Workaround](.agent/skills/general_docker_run/SKILL.md) を参照してください。
...
`docker-check-cpp.sh` を使用して、コンテナ内で安全にルールチェックを行います。

```bash
# 特定のファイルを検証
bash .agent/skills/general_docker_run/scripts/docker-check-cpp.sh src/main.cxx

# ディレクトリ以下の全ファイルを一括検証
find src -name "*.cxx" | bash .agent/skills/general_docker_run/scripts/docker-check-cpp.sh
```
...
### 4.3 スクリプト
- **[check_embedded_rules.py](.agent/skills/embedded_cpp_check/scripts/check_embedded_rules.py)**: 禁止APIの使用を静的解析で検出します。

## 5. 品質・検証ルール

- **不変条件 Invariants**: 本スキルによるチェックを通過したコードは、実行時に暗黙的なヒープ確保や例外送出を行わないことが保証されます。
- **バリデーション**: [embedded_cpp_rule](.agent/rules/embedded_cpp_rule.md) との整合性が自動的にチェックされます。
...
**コンテナ内でマウントが空**:
Windows環境特有の問題です。[Docker Workaround](.agent/skills/general_docker_run/SKILL.md) のトラブルシューティングを参照してください。
