...
SCOPE: プロジェクト全域
RELATED: project_friction_audit, docker_workaround
...
```bash
# 基本形:
bash .agent/skills/general_docker_run/scripts/docker-explore-codebase.sh <command> [args...]

# 例: AST解析 (JSON)
bash .agent/skills/general_docker_run/scripts/docker-explore-codebase.sh ast path/to/file.hxx --json -Iinc
```
...
bash .agent/skills/general_codebase_explore/scripts/explore-codebase.sh summary docs/architecture/
```
...
```bash
# コンテナ内蔵の python を使用:
python3 .agent/skills/general_codebase_explore/scripts/explorer.py summary src/
```

## 3. 解析サブコマンド
- `summary <path>`: ファイルまたはディレクトリのシンボルツリーと骨格を抽出。
- `ast <path>`: Clang AST をダンプ。`--json` フラグ併用可能。
- `graph <path>`: `cflow` を用いたノイズ除去済みコールグラフを生成。
- `symbols <path>`: `clang-check` を用いたクリーンなシンボルリストを取得。
...
```bash
bash .agent/skills/general_codebase_explore/scripts/explore-codebase.sh summary src/main.cxx
```
...
bash .agent/skills/general_docker_run/scripts/docker-run-command.sh find src -name "*.cxx" | head -n 5 | xargs -I {} bash .agent/skills/general_docker_run/scripts/docker-explore-codebase.sh summary {}
```
