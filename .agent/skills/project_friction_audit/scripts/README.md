...
- **影響範囲 (Side Effects)**: `docs/temp/` 下にレポートファイル（Markdown形式）を生成する。

## 2. インターフェース (CLI & Interface)

### [audit_friction.py](file:///w:/mysrc/fireball/.agent/skills/project_friction_audit/scripts/audit_friction.py)
`python3 audit_friction.py [options] [path...]`
- **引数 (Arguments)**:
    - `path`: 監査対象のドキュメントまたはディレクトリ。
- **オプション (Options)**:
    - `--requirements <path>`: `requirement_list.md` のパスを指定。

### [check_traceability.py](file:///w:/mysrc/fireball/.agent/skills/project_friction_audit/scripts/check_traceability.py)
