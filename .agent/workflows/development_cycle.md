...
3. **定義**: エージェントがコンポーネント単位でヘッダファイルにインターフェイスを定義する。
    - インターフェース定義のコーディングスタイル、命名規則、型語彙、コメント規約については、`docs/orders/coding_guidelines.md` を参照すること。
    - また、`docs/oders/patterns/interface.md` の設計原則も守ること。
...
設計で詳細レベルのシステムの一貫性が成立すると、実装を中心としたラピッド開発の回転が始まる。
