...
2. **契約の明文化**: `///` コメントを用いて、事前条件、事後条件、エラー時の挙動を記述する。
3. **自動生成**: `wit_to_cpp.py` を用いて、実装のベースとなるインターフェースヘッダを生成する。
4. **レビューと承認**: ユーザーは **WIT ファイルの内容** をレビューし、構造と契約に合意する。WIT の承認なしに実装コード（.cxx）を生成してはならない。

## 3. 実装フェーズ (Implementation)
1. **スケルトン生成**: 生成されたインターフェース（`inc/gen/*.hxx`）を継承した実装クラス（`src/*.cxx`）を作成する。
