...
- `pytest` / `unittest` / `mock`、`test_*` / `_test_*` の製品シンボル、テスト専用フックやバックドアを `experiments/pysim` の製品コードへ持ち込まない。テスト専用セットアップは `experiments/pysim/qa` 側に置く。
- 必須入力を `T | None` やデフォルト `None` で受けて、呼び出し側のテスト用セットアップ・互換経路を本番コードへ埋め込んではならない。必須メタデータはテストまたは上位の呼び出し側で取得して具象型の引数として渡す。`None` は実行状態として本当に「値がない」ことが仕様上許されるデータだけに限定する。
