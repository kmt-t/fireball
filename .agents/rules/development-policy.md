...
### Step 0: Bonsai Design (盆栽デザイン: 静的設計・動的設計のペアリング)
- 対象領域の仕様書群（`docs/components/**`）に仕様とアーキテクチャの骨格を記述する。
- **静的設計と動的設計を必ずセットで定義する**:
  - **静的設計**: データ構造、メモリ配置、クラス・構造体、内部ブロック図（`graph TD`）。
  - **動的設計**: 状態遷移図（`stateDiagram-v2`）、アルゴリズム手順、相互作用。
- 自然言語（日本語）で概念・意図・不変条件を定義し、特定言語依存の実装型（C++ の内部型や Python の `bytearray` 等）の混入を排除する。
- 動的アルゴリズムの可視化において、**責務が重要な場合はシーケンス図（`sequenceDiagram`）**、**手順が重要な場合はアクティビティ図（`flowchart TD`）** を用いる。詳細な記法規約は `.agents/rules/documentation-standards.md` を参照。
