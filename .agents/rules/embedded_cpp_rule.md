...
*   **例外の禁止**: `try`, `catch`, `throw` は禁止。`result<T, E>` パターンを使用する。
*   **RTTIの禁止**: 実行時型情報は無効化される。
...
| 仕様型名 | C++ エイリアス | 実装 | 説明 |
| :--- | :--- | :--- | :--- |
| バイナリビュー | `binary_view` | `std::span<const uint8_t>` | 読み取り専用ROMビュー |
| 可変ビュー | `mutable_binary_view` | `std::span<uint8_t>` | 書き込み可能RAMビュー |
| データ範囲 | `data_range<T>` | `std::span<T>` | 型付きメモリ範囲 |
| 結果 | `result<T, E>` | Custom | Expectedライクな結果型 |
| オプショナル | `optional<T>` | `std::optional` (or custom) | 値の有無 |
