...
- **検証項目**:
  ...
  4. 自然言語仕様および一文一義・主述近接の徹底（見出し・項目名に C++ や Python の内部型名が混入していないか。1文に複数の命題・条件・理由が数珠繋ぎになっていないか。段落の役割が定義・契約、動作・手順、理由・設計判断、検証の型に沿って分かれ、異なる役割が不必要に混在していないか。正本の `.agents/rules/documentation-standards.md` に従い、役割が切り替わる位置で段落を分け、情報を保持した修正案を示す）。
  5. 仕様書内の図（Mermaid 図）が本文の自然言語解説、アルゴリズム手順、表定義と完全一致しているか（本文改定に伴う図の追随漏れ・陳腐化の排除）。
  6. 仕様書内の例示コード・擬似コード・API使用例が、本文の自然言語解説、アルゴリズム手順、表定義、およびコンセプトコード・WIT定義の最新インターフェースと完全一致しているか（本文改定に伴う例示コードの追随漏れ・陳腐化の排除）。
  7. 契約/実装分割パターン（`document_structure.md` §2.1-4）を適用したコンポーネントの場合、契約側が実装側の具象データ構造・物理レイアウトを記述していないか（CL-05）、Tier再配置後の相互参照にダングリングが残っていないか（CL-06）、上位/下位キーワード表記規約（CL-07）が適用されているか。
  8. **依存方向逆流チェック（CL-08）**: 手順は以下の通り（再現性のため機械的に実行する）:
     a. `docs/architecture/document_structure.md` §1（Tier定義・コンポーネント一覧）と §2.2（上り方向の依存禁止）を読み、本コンポーネントについて「何に依存されているか（被依存関係）」を特定する。Tier上下関係（下位Tierは上位Tierに依存する）に加え、§1.1の一覧やコンポーネント本文中で「〜に依存する」と明記された関係、`{META_ContractImplSplit}` の契約/実装ペアも対象に含める。
     b. 本コンポーネントの仕様書全文から、依存する側（利用側）コンポーネントの名称が登場する箇所を全て洗い出す。
     c. 各箇所について、それが「抽象インターフェース経由で一般的に利用され得る」という記述（許容）か、依存側固有の内部状態・具体的シナリオ・トリガー条件・データ構造を記述している（逆流＝CRITICAL/MAJOR）かを判定する。契約側が「実装の詳細は実装側ファイルを正本とする」と実装側ファイルへポインタを示すだけの記述は逆流ではない。
     d. 逆流と判定した箇所は、依存側ファイル（本来の記述場所）に既に同内容が書かれているか確認した上で（書かれていなければ MAJOR として依存側への移設も提案）、被依存側からは削除し抽象的な記述に置き換えることを提案する。
  9. **不要な参照の排除チェック（CL-09）**: 判定はキーワードではなく文章（本文の言及）を起点に行う。本コンポーネント仕様書中の他コンポーネントへの言及ひとつひとつについて、(a) 上記CL-08の逆流に該当するか、(b) 依存側・正本側に既に書かれている内容の重複・言い換えでないか、(c) その文を削除しても仕様の理解・トレーサビリティが失われないか、を確認する。不要と判定した参照は本文からの削除・簡潔化を提案し、その結果として本文から引用されなくなった `{Keyword}` があれば `keyword_dictionary.md` からの当該行削除も併せて提案する（要求仕様に由来せず他に引用箇所もないキーワードを残さない）。要求仕様・ADR・Challenge由来で実際に意味内容を持つキーワードのリネーム・削除は提案しない。
