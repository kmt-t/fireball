...
ドキュメントレビューでは、内容の正しさだけでなく、文章が人間に追跡可能な構造になっているかを確認します。次のどちらかの形式を満たしていない記述は修正対象です。

- 手順と注意事項を、それぞれ簡潔な箇条書きに分ける。
- 本筋を最初に簡潔に示し、付帯条件を目的別の箇条書きグループへ分ける。

レビュー時は、次の順で確認します。

1. 本文から要求、手順、前提条件、制約、例外、理由、検証方法を抽出する。
2. 本筋を一文または短い手順列として先に配置する。
3. 付帯条件を適用範囲・安全性・正確性・性能・実装補足の順でグループ化し、影響の大きい順に並べる。
4. 分解前に抽出した情報と分解後の箇条書きを照合し、情報の欠落、重複、意味の変化がないことを確認する。
5. 理由は現在の仕様上の根拠に限定し、過去の経緯、作業履歴、変更前の状態を削除する。

段落が長いことだけを理由に情報を削除してはなりません。情報量が多い場合は、見出し、箇条書き、表、手順図へ分割します。レビュー結果には、可読性違反の箇所だけでなく、分解前後の情報照合で欠落していないことも記録します。
