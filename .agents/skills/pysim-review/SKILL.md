---
name: pysim-review
description: experiments/pysim 配下の Python ソースコードを、組み込み C++23 への移植可能性およびプロジェクト設計規約の観点から徹底レビューするスキル。仕様書一致性、型注釈・Any禁止、set/dict/動的配列排除・システムコンテナ強制、計算量・決定論性、不要な後方互換コードの排除、ROM/RAM配置可能性（不変性）、余計なメモリ使用の排除、デッドコード排除、文書階層に基づくコンポーネント分割の9大評価軸を専門サブエージェントで並行監査する。
---
