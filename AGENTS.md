...
※ 議事録は `docs/kaigi` フォルダ以下に日付入りのファイル名で保存すること。
...
5. わいがやの議事録を作成する。
6. 議事録の内容を仕様にフィードバックする。
7. フィードバックした仕様のトレーサビリティを整える。
...
3. 計画と成果物の乖離を分析する。
4. 分析から設計を変えるべきか計画を変えるべきかアクションプランを考る。
5. アクションプランは優先度順に @docs/actions/list.md にまとめる。
6. 計画、設計にフィードバックする。
