...
## 3. 🛠 技術設計・同期プロトコル (Technical Design Protocol)

Fireballプロジェクトにおいて、論理設計（WIT）と物理実装（C++）の整合性を保つための標準手順。

### 🔄 変更と同期の順序
設計変更を行う際は、以下のステップで実施することで、システム全体の不整合を最小化する。
1.  **WIT (Source of Truth) の定義**: インターフェース名から実装詳細を除き、具体的かつ役割が明確な `kebab-case` で定義する。
2.  **C++ インターフェースの同期**: 確定したWITに基づき、`snake_case` を用いてC++ヘッダを更新する。
3.  **規約への反映**: 変更内容に基づき、必要に応じて本ドキュメントや関連規約を更新する。

### 🏷 命名と型定義の原則
*   **記述的な命名 (Expressive Naming)**: `controller` や `manager` などの汎用的な名前を避け、`device-controller` や `task-scheduler` のように、単体で文脈が理解できる具体的な名称を使用する。
*   **メモリ実体（SHM）の優先**: WITにおける `list` や `string` はヒープ確保を誘発するため、データ転送には原則として `shm_id` (共有メモリハンドル) を使用する。

---
*注意: 本ドキュメントは、Fireballの設計品質と実装の生産性を両立するための技術的合意である。*
