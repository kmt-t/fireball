...
class vmtimer {
 public:
  class timespan {
   public:
    uint64_t span() const { return tm_->tick() - start_; }
  ...
  uint64_t tick() const { return tk_; }

  const timespan span() const { return timespan(this); }
...
class global_timer_registory {
 public:
  ...
  static vmtimer* sys() { return &sys__; }

  static void register_countup(uint8_t index, vmtimer* tm) {
    slots__[index] = tm;
  }

  static void unregister_countup(uint8_t index) { slots__[index] = nullptr; }
