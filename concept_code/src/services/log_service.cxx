...
namespace fireball {
	namespace services {
		...
		log_service::log_service() : ringbuffer_(fireball::allocator::sys_allocator::allocator<entry_t>()), last_ts_(0U){
			// nothing.
		}

		coro::task<void> log_service::start_service() const {
			co_return;
		}
		...
		coro::generator<log_service::entry_view_t> log_service::view() const {
			...
			while (true) {
				...
				if (iter->packet.category == CATEGORY_TIME && iter->packet.code == CODE_TIME_ABS) {
			...
			for (; iter != ringbuffer_.end(); ++iter) {
				...
				case CATEGORY_VALUE_TAG_EOS:
					switch (iter->packet.code) {
					case CODE_TYPE_S32:
						co_yield value_view_t{ curr_ts, iter->packet.arg, static_cast<int32_t>(0U) };
						break;
					case CODE_TYPE_U32:
						co_yield value_view_t{ curr_ts, iter->packet.arg, static_cast<uint32_t>(0U) };
						break;
					case CODE_TYPE_F32:
						co_yield value_view_t{ curr_ts, iter->packet.arg, std::bit_cast<f32_t, uint32_t>(0U) };
						break;
					}
					break;
				case CATEGORY_VALUE_LOWWORD_EOS:
					...
					switch (iter->packet.code) {
					case CODE_TYPE_S32:
						co_yield value_view_t{ curr_ts, iter->packet.arg, static_cast<int32_t>(iter->packet.arg) };
						break;
					case CODE_TYPE_U32:
						co_yield value_view_t{ curr_ts, iter->packet.arg, static_cast<uint32_t>(iter->packet.arg) };
						break;
					case CODE_TYPE_F32:
						co_yield value_view_t{ curr_ts, iter->packet.arg, std::bit_cast<f32_t, uint32_t>(0U) };
						break;
				...
				case CATEGORY_VALUE_HIGHWORD_EOS:
					...
					switch (iter->packet.code) {
					case CODE_TYPE_S32:
						co_yield value_view_t{ curr_ts, iter->packet.arg, static_cast<int32_t>(val | (iter->packet.arg << 24U)) };
						break;
					case CODE_TYPE_U32:
						co_yield value_view_t{ curr_ts, iter->packet.arg, static_cast<uint32_t>(val | (iter->packet.arg << 24U)) };
						break;
					case CODE_TYPE_F32:
						co_yield value_view_t{ curr_ts, iter->packet.arg, std::bit_cast<f32_t, uint32_t>(val | (iter->packet.arg << 24U)) };
						break;
					}
					break;
				default:
