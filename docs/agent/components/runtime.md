...
ゲスト、サービスの権限をTierで分離する。ゲストよりサービスのほうを上位のTierにすることでゲストのTierを下げることができる。

| Tier | Description |
| --- | --- |
| 0 | libc、WASI、ガベージコレクションへのアクセス |
| 1 | 改変不可サービスへのアクセス |
| 2 | vSoC API、ランタイムAPIへのダイレクトアクセス |

ゲストの権限は起動時にランタイム内に保存されるためランタイム側で検証することができる。
