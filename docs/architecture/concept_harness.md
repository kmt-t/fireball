...
本アーキテクチャは、依存関係をC++20/23 Conceptで定義し、コンパイル時にテンプレート引数として注入することで、**ゼロコスト抽象化**と**完全なテスト可能性（モック注入）**を両立させる。
...
| 方式 | 呼び出しコスト [見積] | メモリコスト [見積] | インライン化 |
|:---|:---:|:---:|:---:|
| 仮想関数（vtable） | 間接呼出 (2-5 cycles) | vptr (+4〜8B/object) | ❌ 不可 |
| **Conceptベースハーネス** | **0 cycles (直接インライン)** | **0B (メモリ追加なし)** | ✅ **可能** |

※ 上記は C++ テンプレート/Concept による静的ディスパッチの理論特性に基づく設計目標値（[目標値]）。ベンチマーク評価は Phase 1 で実施予定。
