...
下位 Tier の文書が、参照・具体化する上位 Tier の文書と矛盾する場合は、**常に上位 Tier 側を正とする**。Clean Architecture では、方針（policy）は内側の層が定義する。詳細（detail）は外側の層が方針に従って実装する。詳細から方針を決めてはならない。

Fireball の Tier 構造では、上位 Tier が粗粒度の方針を示す。下位 Tier は具体化された詳細を記述する。この階層では、矛盾を**下位側の記述誤りまたは追随漏れ**として扱う。上位側を下位側に合わせて書き換えてはならない。
...
- 「上位 Tier の記述が古い／実装しづらい」という理由で下位 Tier 側の記述を正として上位を書き換えることはしない。上位を変更すべきだとコンポーネント作業者が判断した場合は、それ自体を独立した提案として明示し、黙って下位からの逆流で上書きしない（上り方向の依存禁止の原則に従う）。
...
- 要求・ADR・Challenge・コンポーネント仕様の意味キーワードを、参照の都合だけで `LINK-<連番>` に置き換えない。意味を持たない純粋リンクに限り `LINK-<連番>` を使う。
