...
| `{TrackableBlockMask}` | `jit_runtime.md` | `jit_runtime.md` | ロード時に一度だけ確定する 1-bit ブロック追跡可否マスク。カードマーキング表の更新対象かをディスパッチ時に $O(1)$ 判定し、ブロックの静的メタデータを毎回参照し直さない | Scenario 4, 5 (`JITR-GOTCHA-07`) |
