...
## 7. 設計完了チェックリスト（網羅性確認）
コンポーネントの定義が完了したとみなすためのチェックリスト。確認してチェックすること。

- [ ] コンポーネントの責務が明確に定義されているか
- [ ] 設計が実装詳細（言語仕様）に依存せず、目的・意図と課題を解消するコンセプトが定義されているか
- [ ] データ構造・API の見出しおよび項目名がC++識別子でなく自然言語（日本語）で記述されているか
- [ ] コードブロックが存在する場合、Python（擬似コード）であり C++ の実装コードでないか
...
- [ ] 上位の要求 `{Keyword}` ととのトレーサビリティが確保されているか
- [ ] 直交表を用いて公開APIの各パラメータや内部状態の組み合わせ網羅性が検討されているか
