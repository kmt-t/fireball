...
コンポーネントが常に満たすべき不変条件を列挙する。各条件は数学的に或いは述語論理で表現することが望ましい。

| 不変条件 | 説明 | 範囲 | 検証方法 |
| :--- | :--- | :--- | :--- |
| 例: キュー一貫性 | キューの挿入・削除前後でサイズが符号付き整数であること | グローバル | TLA+ 状態不変式 |
| 例: 所有権単調性 | リソース所有権が「移譲中」を経由し、二重所有が発生しないこと | データ移動フロー | TLA+ Init-Next-Spec |
