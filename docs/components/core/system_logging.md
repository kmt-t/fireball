...
## 1. コンセプト `{IPCRouter}` `{DictionaryBasedIPC}` `{BufferedLogging}` `{IdleDetection}`
...
#### `logging_config` `{ConfigurableSystem}`
...
### 4.1 アルゴリズム `{DictionaryBasedIPC}` `{BufferedLogging}`
...
### 4.3 COOS Idle Hook 連携 (Flush Protocol) `{IdleDetection}`
...
### 4.4 状態遷移図 `{DictionaryBasedIPC}` `{BufferedLogging}` `{IdleDetection}`
```mermaid
stateDiagram-v2
    [*] --> Idle
...
### 4.5 内部シーケンス `{DictionaryBasedIPC}` `{BufferedLogging}` `{IdleDetection}`
#### ログ出力シーケンス
```mermaid
sequenceDiagram
...
### 5.2 URI/IPCインターフェイス `{DictionaryBasedIPC}`
