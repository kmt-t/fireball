...
- **依存関係解決**: インポートセクションをスキャンし、必要なモジュール名とエクスポート名（関数ID/グローバルID等）を抽出し、`module_registry` を介して他モジュールの `lookup_export` とリンクする。
- **軽量検証 (Lightweight Verification)**: 以下の項目をチェックする。 `{LightweightVerifier}`
    - マジックナンバー (`\0asm`) およびバージョン (`1.0`)。
    - 各セクションのサイズとバイナリ末尾の整合性。
    - セクション順の正当性 (Customセクション以外は昇順)。
    - インポート/エクスポートの型整合性（シンボル解決レベル）。
