...
## 1. コンセプト `{LowLatencyJIT}` `{MemoryIsolation}` `{FaultIsolation}` `{EnvironmentPointer}`
...
#### `vsoc_config` `{ConfigurableSystem}`
...
### 4.1 アルゴリズム `{ThreadedInterpreter}` `{JIT_CopyAndPatch}` `{Challenge_ApproximateYield}` `{JIT_Safepoint}` `{Debugger_Jit_Flush}`
...
### 4.2 状態遷移図 `{ThreadedInterpreter}` `{JIT_CopyAndPatch}` `{Challenge_ApproximateYield}` `{JIT_Safepoint}` `{Debugger_Jit_Flush}`
```mermaid
stateDiagram-v2
    [*] --> Idle
...
### 4.3 内部シーケンス `{ThreadedInterpreter}` `{JIT_CopyAndPatch}` `{Challenge_ApproximateYield}` `{JIT_Safepoint}` `{Debugger_Jit_Flush}`
#### WASM実行およびJIT遷移シーケンス
```mermaid
sequenceDiagram
...
#### マルチモジュール動的リンクシーケンス `{MultiModule_Support}`
...
#### `notify-interrupt` `{RecoveryStrategy}`
| 項目 | 内容 |
| :--- | :--- |
...
#### `register-hook` `{vMMIO_TrapAndEmulate}`
| 項目 | 内容 |
| :--- | :--- |
...
### 5.2 ネイティブAPI エクスポート `{NativeAPI_Export}`
...
### 5.3 マルチモジュール対応 `{MultiModule_Support}`
...
### 5.4 URI/IPCインターフェイス `{RecoveryStrategy}` `{vMMIO_TrapAndEmulate}` `{NativeAPI_Export}` `{MultiModule_Support}`
- **URI**: `fireball://vsoc/control/<instance_id>`
- **メッセージ形式**: 実行制御、状態取得用のKey-Valueプロトコル。詳細定義は IPCルータの仕様に準ずる。

### 5.5 関連コンポーネントとの連携 `{RecoveryStrategy}` `{vMMIO_TrapAndEmulate}` `{NativeAPI_Export}` `{MultiModule_Support}`
...
### 6.3 安全性制約と方策 `{MemoryBoundaryCheck}` `{RestrictedPhysicalAccess}`
