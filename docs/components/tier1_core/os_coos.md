...
各サブコンポーネントおよび通信リソースは、専用アロケータが管理する領域に配置し、ソフトウェア上の所有権と割当境界を分離する。ハードウェアによるメモリ保護方式と物理領域配置は対象プラットフォームで定め、ARMv8-MではTBDとする。
...
ムーブ専用ペイロードの sender/reply stamper と Grant/Claim は、Scheduler が対象 TCB を認証した実行コンテキストで実行する。通信層は共有メモリ所有権を直接変更しない。
