...
| `FB_CONF_JIT_CARD_SHIFT` | JITカードテーブルのビットシフト数（関数ごと、4バイト単位 = 2） | `2` | `{META_ConfigurableSystem}` |
...
| `FB_CONF_VMMIO_MAX_PTES` | FlatMap ページテーブルに保持可能な PTE の最大件数 | `64` | `{META_FlatMapIndexed}` `{GLOBAL_StaticScalability}` |
