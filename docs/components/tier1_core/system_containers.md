...
| 単調縮小不変条件 | **設計理由と不変条件 (`CONT-GOTCHA-02`, `{FlatViewNarrowing}`)**: ビューのスライスおよびナローイングは親ビューの区間（`[self.first, self.last]`）を単調縮小（縮小または同等）させることのみを許可し、親の境界外（`first < self.first` または `last > self.last`）を指定した拡張は例外またはアサーション違反により厳格に遮断される。ビューが親スパンの境界を超えて拡張できると、外部メモリ領域への不正アクセス（バッファオーバーラン）を引き起こすため、ビューの縮小性を静的不変条件として強制する |
