...
<!-- evidence:
     ...
     test: docs/qa/tier1_core/system_containers_test_spec.md
-->
...
本コンポーネントは、Fireball 全体で共有されるコンテナ語彙を定義する。複数の Tier が同じ参照パターンを必要とするため、各所で個別に説明せず本書を型定義の正本とする。
...
| `ReadOnlyRadixBinaryTreeView` | 「キー `K` に対応する値は何か（基数＋二分探索）」 | **Radix Table** ($O(1)$) + **有界二分探索** $O(\log n)$ | WASMローダのファイル位置・シンボル索引 |
...
- **アロケータからのメモリ供給とライフサイクル管理 (`{Runtime_BumpAllocator}`, `{OneRuntimeOneGuest}`, `{System_Allocator}`)**:
  各ストレージコンテナは、外部から渡された連続メモリ領域（`std::span` またはアロケータにより切り出されたバッファスライス）を受け取ってゼロアロケーションで初期化できる。ストレージのメモリ供給とライフサイクルは、使用する層（ゲストまたはシステム基盤）に応じて以下のように直交分離される。
  - **ゲスト層（WASMモジュール内）**: 1ランタイム1ゲスト原則（`{OneRuntimeOneGuest}`）のもと、モジュールロード時に親ランタイムの専用バンプアロケータ（`{Runtime_BumpAllocator}`）から各コンテナの裏打ちストレージ（RAM/XN）が順次切り出される。モジュール破棄時は個別オブジェクトの破棄走査を行わず、アリーナごと $O(1)$ で一括解放される。
...
C++23 のこれらはコンテナアダプタであり、既定の下位コンテナが `std::vector` であるため および （無制約な動的再確保に伴うレイテンシ揺らぎとメモリ断片化の排除）に抵触する。本アーキテクチャでは、用途別アロケータまたは固定長バッファ＋`count` 追跡による決定論的かつ安全なストレージを採用する。

## 2. アーキテクチャ分類
<!-- traceability: {META_3TierSeparation} {Type_Vocabulary} -->
本コンポーネントは **Tier 1 (主要システムコンポーネント: Primary Component)** に属する。実行時の振る舞いも状態も持たないヘッダオンリーの型語彙（Vocabulary）であり、各 Tier（Tier 2 の vMMIO、Tier 3 の JIT エントリ検索・カードマーキング等）は本書で定義された非所有ビュー型（`flat_map_view`, `flat_set_view`, `radix_binary_tree_view`, `bit_view`）を利用して具象データを操作する。JITエントリ集合は少数の疎キーであるため`flat_map_view`を使い、Radix索引はローダ等の大規模索引に限る。本書は下位 Tier の具象データ構造や内部ヘッダには一切依存せず、純粋な型語彙の提供に専念する。

## 3. 静的モデル

### 3.1 データ構造
<!-- traceability: {META_FlatMapIndexed} {META_BinarySearch} {FlatViewNarrowing} {PackedBitView} {GLOBAL_StaticScalability} -->
- **`flat_map_view<Key, Value>`**: **AoS（Array of Structures）構造**を採用し、昇順ソート済みのエントリ配列（`std::span<const flat_map_entry<Key, Value>>`）の非所有ビューで構成される疎マップ。キーと値のペアを単一配列で連続保持し、C++ 標準の `std::sort`（`constexpr` 対応）や `std::lower_bound` をそのまま利用できる。保持メンバは単一スパン（ポインタと長さの計 2 ワード = 1組）のみであり、レジスタ渡しに最適化されている。
- **`flat_set_view<Key>`**: 昇順ソート済みのキー列のみを指す非所有ビュー。所属判定を行う。
- **`radix_binary_tree_view<Key, Value, RadixShift>`**: 基数テーブル（Radix Table / $O(1)$ スカラー開始インデックス配列：サイズ $K+1$、バケット $p$ の区間は `[table[p], table[p+1]]`）とソート済みエントリ配列の局所二分探索をカプセル化した多段索引ビュー。
- **`bit_view<Bits>`**: 1 要素が 1 バイト未満の密なビット詰め表を指す非所有ビュー。添字で直接読み書きする。
...
AoS（Array of Structures）構造に基づく、昇順ソート済みの (Key, Value) ペア配列に対する非所有ビュー。粗索引による範囲絞り込みと二分探索を合成し、$O(\log N)$ でキーを特定して対応する値を取り出す。キーと値を単一配列で連続保持するため、値の型やサイズに関わらず保持メンバは単一スパン（ポインタと長さの計2ワード）のみで済み、レジスタ渡しに最適化される。
...
`flat_set_view` を `flat_map_view` のダミー値付き特殊形として実装せず独立した型としているのは、不要な値列ポインタの保持によるメモリおよびレジスタ浪費を完全に防ぐためである。キー配列のポインタと長さ（計2ワード）のみで構成することで、AAPCS レジスタ2個での完全な値渡し（FastCall）とゼロコスト抽象化を実現する。
...
**共通の設計上の性質（map / set）**:
- **ゼロコスト**: 実体はポインタと長さのみ（map も set も 1 組 = 2ワード）。所有も確保も行わず、AAPCS の引数レジスタに載る大きさに収まる。
...
RAM 32KB という制約下では、数値しか取らない状態表を 1 バイト/要素で持つのは無視できない浪費になるため、本語彙はビット幅を型の一部として扱う。
...
**設計上の制約**:
- `Bits` は 1, 2, 4 のいずれかに限る (`GOTCHA-CONT-01`)。8 の約数のみを許すことで 1 要素が高々 1 バイトに収まり、読み出しが単一のロードとシフト・マスクで完結する（バイト跨ぎの結合処理が不要になる）。**設計理由と不変条件**: 3bit や 5bit など 8 の約数以外のビット幅を許容すると、単一要素が複数バイトに跨がるアライメント問題が発生し、1 回の要素アクセスに 2 回のメモリアクセスと複雑なビット合成が必要となる。これによりロード性能が著しく劣化し、アトミック性も破壊される。8 の約数に制限することで、任意の要素アクセスが常に「単一バイトロード＋ビットシフト＋マスク」の定数時間 $O(1)$ 命令列で完結することを保証する。この制約は `static_assert` で強制する。8 ビット以上の密な表には本型を用いず、素の `std::span<T>` を使う。 `{META_CompileTimeValidation}`
- `slice()` の開始添字にバイト境界の制約を課さない。ビット原点フィールドが端数を吸収する。
- 読み出しは `(byte >> shift) & mask` に展開される。`Bits` がコンパイル時定数であるため `shift` と `mask` は即値となり、実行時の除算・剰余は発生しない。
...
- **AoS 標準ソートと二分探索 (Standard Sort & Binary Search)**: 自前のソート関数（連動ヒープソート等）の車輪の再発明を排し、C++ 標準の `std::sort`（C++20 `constexpr` 対応）および射影付き `std::lower_bound` をそのまま利用する。小規模組み込み（$N \le 64$）においてデータ全体が 1〜2 本のキャッシュライン（32〜64 バイト）に収まるため、AoS でキャッシュミスは発生せず、標準ライブラリの極限まで最適化されたアルゴリズムの恩恵を最大化できる。 `{META_ZeroCostAbstraction}`
- **検索方式の選択**: 読み取り量とエントリ数が大きく、全域二分探索が広い範囲を読む表には `RadixBinaryTreeView` を使い、基数表で狭い連続範囲へ絞ってキャッシュ汚染を抑える。エントリ数が小さい表は基数表を持たず `FlatMapView` の二分探索を使う。アクセス局所性がある固定表は、キーをXOR折りたたみして選ぶダイレクトマップキャッシュを併用する。
...
def lookup_jit_entry(
    ...
    """JIT entry lookup: O(1) card prefilter followed by sparse-key binary search."""
...
| 単調縮小不変条件 | **設計理由と不変条件 (`GOTCHA-CONT-02`, )**: ビューのスライスおよびナローイングは親ビューの区間（`[self.first, self.last]`）を単調縮小（縮小または同等）させることのみを許可し、親の境界外（`first < self.first` または `last > self.last`）を指定した拡張は例外またはアサーション違反により厳格に遮断される。ビューが親スパンの境界を超えて拡張できると、外部メモリ領域への不正アクセス（バッファオーバーラン）を引き起こすため、ビューの縮小性を静的不変条件として強制する |
...
| :--- | :--- | :--- | :--- |
| JIT エントリ索引 (`jit_runtime_entry`) | ソート済み固定容量配列 | WASM PC からネイティブコードへの変換 | Radixなし。少数エントリの二分探索 $O(\log n)$ |
...
- **方策**: 大規模なWASMローダ索引ではRadixで候補区間を狭めてから二分探索し、局所性を保つ。少数のJITエントリはRadix表を設けず、ソート済み固定容量配列を二分探索する。カード状態の判定は密な`bit_view`で探索せず、単一のロードとシフト・マスクで完了する。
...
- **方策**: キー区間と値区間をビューとして束ねることで、両者を取り違えた組み合わせをそもそも表現できなくする。絞り込み操作の単調縮小性により、合成した絞り込みが区間外へ広がることはない。`Bits` を 8 の約数に `static_assert` で限定することで、1 要素がバイトを跨ぐ場合の部分書き込みを設計から排除する。探索を持つ型と持たない型、値を返す型と返さない型を分離することで、密な状態表を誤って探索対象として扱う設計や、存在しない値列への参照を型で阻む。
