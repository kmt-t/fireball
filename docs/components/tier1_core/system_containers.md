...
`flat_set_view` を `flat_map_view` のダミー値付き特殊形として実装せず独立した型としているのは、不要な値列ポインタの保持によるメモリおよびレジスタ浪費を完全に防ぐためである。キー配列のポインタと長さ（計2ワード）のみで構成する。物理引数レジスタへの割当は対象ABIに従い、ARMv8-MではTBDとする。
...
**共通の設計上の性質（map / set）**:
- **ゼロコスト**: 実体はポインタと長さのみ（map も set も 1 組 = 2ワード）。所有も確保も行わず、引数レジスタへの載せ方は対象ABIに依存し、ARMv8-MではTBDとする。
...
状態表のメモリ密度を明示し、保持量を用途に応じて抑えるため、本語彙はビット幅を型の一部として扱う。実機ごとの容量適合性は対象プラットフォームで検証する。
...
**設計上の制約**:
- `Bits` は 1、2、4 のいずれかに限る (`GOTCHA-CONT-01`)。8 の約数だけを許すため、1 要素は最大 1 バイトに収まる。読み出しは単一のロードとシフト・マスクで完結し、バイトを跨ぐ結合処理は不要である。**設計理由と不変条件**: 3bit や 5bit などを許すと、1 要素が複数バイトに跨がる。アライメント問題が生じ、1 要素の読み出しに複数回のメモリアクセスと複雑なビット合成が必要になる。これによりロード性能が低下し、アトミック性も損なわれる。8 の約数に制限すると、要素アクセスは「単一バイトロード＋ビットシフト＋マスク」の定数時間 $O(1)$ 命令列で完結する。この制約は `static_assert` で強制する。8 ビット以上の密な表には本型を用いず、`std::span<T>` を使う。 `{META_CompileTimeValidation}`
...
小規模組み込み（$N \le 64$）では、データ全体が1〜2本のキャッシュライン（32〜64バイト）に収まる。そのため AoS でキャッシュミスは発生しない。標準ライブラリの最適化されたアルゴリズムを活用できる。 `{META_ZeroCostAbstraction}`
