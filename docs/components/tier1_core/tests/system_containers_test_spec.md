...
| CONT-12 | `StaticFlatMap`の標準ソート（AoS、constexpr対応） | 未ソートのペア配列 | `map.sort()` | C++標準の`std::sort`相当でキー昇順にソートされ、自前ソート関数を抱えずに`is_sorted()`がTrueになり`view().find()`で全要素探索可能 | §3.3, §4.1 `static_flat_map::sort` |
| CONT-13 | `StaticFlatMap`のソート維持挿入・削除 | 構築済みマップ | `insert(k, v)` / `remove(k)` / `erase(k)` | 任意順序での挿入・削除後も常にペア配列の昇順ソート状態が維持され、二分探索の不変条件が保たれる | §3.3 `static_flat_map::insert/erase` |
