...
参考実装: なし
現行実装: `experiments/pysim/system.py`（`_wasi_fd_write`等、直接呼び出し方式 — §3参照）
...
**重大**: `experiments/pysim/system.py`の`_wasi_fd_write`は、system_service.md §4.4が定義する「IPCルータ経由の非同期変換＋co_yield待機」という設計を採用せず、`self.console.write(data)`を**直接同期呼び出し**している。これは:
- SVC-03（IPCメッセージへの変換）: 不採用。
- SVC-04（`resolve_wasi_fd_to_channel`による仮想FDテーブル）: 存在しない。pysimは`fd in (1, 2)`の直接分岐のみ。
- SVC-07（キュー満杯時の`WASI_ERRNO_IO`）: 該当パスが存在しないため不可能。
- SVC-08（co_yieldによるサスペンド）: `fireball_call`が同期的なネイティブ呼び出しである以上、そもそも実装できない（README「Missing spec coverage」に記載のSYS_YIELDと同根の制約）。

この差は、pysimが「システムコールを直接処理する単純なホスト実装」を採用しているのに対し、正本は「WASI呼び出しをIPC経由の非同期メッセージに変換し、サービスタスクが処理する」という、より疎結合なアーキテクチャを要求していることに起因する。**根本的なアーキテクチャの相違であり、単純な機能追加では埋まらない**。
