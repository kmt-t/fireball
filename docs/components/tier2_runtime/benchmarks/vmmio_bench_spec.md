...
vMMIO 仮想アドレス空間（Bit 31 == 1, Stage 2/3）における、ダイレクトマップ方式ソフトウェア TLB（32エントリ, Folding XOR Hash）の $O(1)$ キャッシュヒット性能、TLB ミス時の FlatMap（ソート済み PTE 配列, `fireball::flat_map_view`）二分探索ルックアップ時間、および各 Function Code（FC=12 静的デバイス / FC=14 SHM / FC=15 PASSTHROUGH）のディスパッチレイテンシを計測する。 `{vMMIO_TrapAndEmulate}` `{PhysicalPassthrough}` `{DynamicMmap}` `{UnifiedAccessModel}` `{META_FlatMapIndexed}` `{META_RestrictedPhysicalAccess}`
...
| **BENCHMARK-VMMIO-03** | TLB ミス $\to$ FlatMap 探索 & リフィル ($O(\log N)$) | 32ページ循環アクセス (TLB容量超過) | ns/walk, M ops/sec | 二分探索による安定した PTE 解決 | `{META_FlatMapIndexed}` |
...
2. **TLB ミス & FlatMap 探索測定**:
   - TLB 容量（32エントリ）を満たす 32 ページをストライド走査し、TLB ミスに伴う FlatMap（`vmmio_ptes`）二分探索とリフィルのオーバーヘッドを計測。
3. **セキュリティゲート測定**:
