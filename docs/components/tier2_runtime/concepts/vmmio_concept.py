...
class VMMIOController:
    ...
    def __init__(self, guest_ram_size: int = 8192):  # FB_CONF_GUEST_RAM_SIZE
        ...
        # Direct-mapped TLB: 16 slots, keyed by a 4-bit Folding XOR Hash over 20-bit VPN.
    ...
    def tlb_index(vpn: int) -> int:
        """
        4-bit Folding XOR Hash over 20-bit VPN: 20 -> 10 -> 5 -> 4.
        """
        temp = vpn ^ (vpn >> 10)
        temp = temp ^ (temp >> 5)
        temp = temp ^ (temp >> 1)
...
def test_tlb_hit_after_first_walk() -> None:
    ctrl = VMMIOController()
...
def test_linear_ram_is_bounds_checked_not_waved_through() -> None:
...
def test_linear_ram_bound_check_works_for_non_power_of_two_size() -> None:
...
def test_flatmap_pte_registration_and_tlb_caching() -> None:
...
def test_tlb_slot_conflict_eviction() -> None:
    ...
    for cand in range(1, 64):
        ...
        if ctrl.tlb_index(cand_vpn) == target_slot:
            vpn_b = cand_vpn
            break
    assert vpn_b is not None, "must find a colliding VPN within 64 candidate pages"
