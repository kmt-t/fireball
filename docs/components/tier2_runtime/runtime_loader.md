...
WASM ローダは、ROM 上の WASM32 バイナリを解析し、実行環境向けの索引構造（ModuleView）を生成する。ROM 上のデータを直接参照し、RAM への全展開を避けてメモリ消費を抑える。固定長メタデータは内部レジストリ（`decoded_entity_registry`）に格納する。Element / Data の個別初期化定義は配列化しない。セクション位置から callback で逐次処理する。

**WASM ファイル内のバイト位置（データオフセット）をキーとする検索**には、`RadixBinaryTreeView`（`fireball::radix_binary_tree_view`）を用いる。粗粒度インデックスは $O(1)$ である。絞り込んだ範囲を $O(\log n)$ で二分探索する。全体の確定時間は $O(\log N)$ である。

**インポートテーブルとエクスポートシンボルの検索**では、FNV-1a 32-bit のシンボル名ハッシュをキーに `RadixBinaryTreeView` で候補を絞る。候補ごとに ROM 上の元文字列を照合する。索引探索は $O(1) + O(\log n)$ である。衝突照合を含む最悪計算量は $O(1) + O(\log n) + O(L)$ である。
本設計の動作モデルと軽量検証スコープ（V1〜V6）は、コンセプトコードで検証されている。対象には、ハッシュと RadixBinaryTreeView によるシンボル・インポート検索が含まれる。ファイル位置の逆引きと、`save` / `restore` によるバンプアロケータのトランザクション保護も含む。検証コードは [`loader_concept.py`](docs/components/tier2_runtime/concepts/loader_concept.py) である。
...
- **パースイベントコールバック**: Type、Import、Table、Memory、Global、Export、Function、Startの各定義は、セクションパーサが直接モジュール配列を操作せず、固定のパースイベントコールバックへ通知する。コールバック受信側が保持対象を選択できるため、保持不要な可変長定義を配列化しない。
...
- **Element/Data初期化のストリーム処理**: ElementセクションとDataセクションは個別エントリの配列を保持せず、ROM上のセクション位置とサイズだけを保持する。パース時は検証callbackへエントリを逐次渡す。起動時は適用callbackへエントリを逐次渡す。`global.get`を含む初期化式は起動時の実値で解決する。
...
| 項目 | 内容 |
| :--- | :--- |
| 機能概要 | モジュールの線形メモリとテーブルを確保し、ROM上の初期化定義をストリーミングで適用する。 |
| シグネチャ | `load(module: wasm-module-view) -> operation-result` |
| 引数 | `module`: 展開対象のモジュールビュー |
| 事前条件 | モジュールが `prepare` によりハースネスに登録済みであること。 |
| 事後条件 | モジュールのリニアメモリおよびテーブル領域がゲストRAMに確保・初期化され、Element/Dataの個別エントリ配列を保持しない。 |
