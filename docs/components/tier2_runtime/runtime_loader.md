...
- **基本ブロック適格性静的評価 & JIT 候補ビットマップ生成 (`{JIT_StaticBenefitScoring}`, `{JIT_CandidateBitmap}`)**:
  ...
  **設計理由と不変条件**: JIT キャッシュ（2KB $\times$ 3面）の物理容量とコンパイル・トレース管理オーバーヘッドは有限である。実行時ホットスポット検出に先立ち、投資対効果（加速利得 $\ge$ コンパイルコスト＋マージン）が数理的に保証されるブロックのみを静的選別することで、インタープリタ実行ループにおける非対象ブロックの `HotspotBitmap.touch(pc)` および履歴記録コストを完全バイパス（$O(1)$ ゼロ化）し、キャッシュ汚染とスラッシングを根本から防止する。

##### 命令別短縮利得スコア定義台帳 (`opcode_benefit_table` / `int4_t`)
<!-- traceability: {JIT_StaticBenefitScoring} {JIT_CandidateBitmap} -->
ROM 上に配置される 128 バイトルックアップテーブル（256 オプコード $\times$ 4-bit 符号付き整数 `int4_t`: `-8`〜`+7`）のスコア配分表を以下に定める。JIT コンパイル対象命令（全 48 命令）はインライン展開による加速利得に応じて正のスコアが与えられ、構文デリミタは中立（0）、真のインタープリタ委譲・フレーム生成命令は負のペナルティ、未サポート・トラップ命令は最大ペナルティ（-8）となる。
...
| **`+6`** | 約 +12〜13 命令短縮 | 整数乗算・32bit 比較演算（全11種）・ローカル変数/定数ロード・ビットカウント | `i32.mul` (`0x6C`), `i32.eqz` (`0x45`), `i32.eq` (`0x46`), `i32.ne` (`0x47`), `i32.lt_s` (`0x48`), `i32.lt_u` (`0x49`), `i32.gt_s` (`0x4A`), `i32.gt_u` (`0x4B`), `i32.le_s` (`0x4C`), `i32.le_u` (`0x4D`), `i32.ge_s` (`0x4E`), `i32.ge_u` (`0x4F`), `i32.clz` (`0x67`), `i32.ctz` (`0x68`), `i32.popcnt` (`0x69`), `i32.const` (`0x41`), `i64.const` (`0x42`), `local.get` (`0x20`), `local.set` (`0x21`), `local.tee` (`0x22`) |
...
| **`+4`** | 約 +8 命令短縮 | 整数除算・グローバル変数アクセス・メモリサイズ・スタック選択・復帰・ニュートラル | `i32.div_s` (`0x6D`), `i32.div_u` (`0x6E`), `global.get` (`0x23`), `global.set` (`0x24`), `memory.size` (`0x3F`), `select` (`0x1B`), `return` (`0x0F`), `drop` (`0x1A`), `nop` (`0x01`) |
...
| **`-8` (MIN)** | 最大ペナルティ | トラップ命令・未サポート・ハードウェア非対応演算（JIT 化不適格） | `unreachable` (`0x00`), `f32.*`, `f64.*`, 未定義オプコード |
