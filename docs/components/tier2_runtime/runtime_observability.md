...
本コンポーネントは、VM が実行経路を観測するための Tier 2 契約を定義する。観測対象はゲスト関数の開始・終了、ホスト呼出、JIT 境界、トラップ、停止点である。ゲストプロファイラはこの契約を利用する Tier 3 プラグインとして実装する。

本契約は観測イベントの意味と生成位置だけを定義する。イベントの集計、ログ形式、固定長リング、外部搬送は Tier 3 プラグインが担当する。Tier 2 は特定のログ形式や物理接続へ依存しない。

Profiler と Debugger は目的を分離する。Profiler は VM 状態を読み取るだけの観測者であり、実行を停止・再開・書き換えない。具体的な集計は [`guest_profiler.md`](docs/components/tier3_plugins/guest_profiler.md) が担う。Debugger は別の `ExecutionControl` 契約を利用して実行状態を変更する。Debugger の停止通知を観測イベントとして発行することは許可するが、Profiler が停止処理を代行することは禁止する。
...
- **`vm_observer`**: VM から固定形式イベントを受け取る読み取り専用契約である。
- **`vm_event`**: 1 件の観測を表す固定幅レコードである。イベント種別、Runtime 識別子、ゲスト関数識別子、ゲスト PC、時刻、呼出相関、状態フラグ、補助値を持つ。
- **`event_sink`**: `vm_observer` へイベントを届ける静的結線である。実装は NullSink、ProfilerSink、LogSink のいずれかを選択できる。
...
```mermaid
graph TD
    VM[VM 実行境界] --> Hook[Tier 2 VM Hooks]
    Hook --> Record[Fixed-width vm_event]
    Record --> Sink[Static Event Sink]
    Sink --> Null[Null Observer]
...
命令ごとのイベントは標準フックに含めない。命令単位の観測が必要な Tier 3 実装は、構成で明示した専用フックまたはサンプリング機構を使用し、通常の VM ホットパスへ常時コストを追加してはならない。
...
- NullObserver はイベント内容を読み取らず、イベント生成をコンパイル時に除去可能である。
...
| 項目 | 内容 |
| :--- | :--- |
| 機能概要 | VM 実行境界で固定幅 `vm_event` を観測シンクへ発行する。 |
...
### 6.1 性能制約と方策
- 通常のフックは固定幅レコードの書込みだけを行い、コールグラフ探索やログ文字列生成を行わない。
- `function_enter` と `function_exit` は共通 `call` テンプレート境界から発行し、Interpreter と JIT の両方へ同じ意味を適用する。
- 観測無効時は NullObserver を静的結線し、イベントペイロード計算を省略する。
...
### 7.2 既知の制限や仮定
本仕様は VM フックとイベント意味論を定義する。Profiler のコールグラフ集計、時間計算、外部ログ形式、物理搬送は [`guest_profiler.md`](docs/components/tier3_plugins/guest_profiler.md) へ委譲する。サンプリングプロファイラ、命令単位トレーサ、ホスト OS の外部プロファイラは本契約の必須機能ではない。

## 8. 設計上の未決事項
- 固定幅 `vm_event` のビット幅と時刻単位を対象 MCU の最小構成へ合わせて確定する。
