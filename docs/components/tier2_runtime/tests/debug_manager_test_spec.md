...
| TEST-DBG-06 | ゲストメモリ書き込み (`M`) と JIT Flush | JITキャッシュにトレース常駐 | `M <addr>,<len>:<hex>` 送信、および [`vsoc_cache_coherency_model.py`](../formal/vsoc_cache_coherency_model.py) の通常モデル・`guards=False` 変異モデルを実行 | メモリが上書きされ `OK` が返却されるとともに、**JIT キャッシュ全バンクが無効化（Flush）** される。通常モデルでは `debugger_memory_write_invalidates_stale_traces` が成立し、変異モデルでは反証される | `{Debugger_Jit_Flush}`, `debugger_memory_write_invalidates_stale_traces` |
