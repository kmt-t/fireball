...
### ファイル内データ位置逆引き & RadixBinaryTree 索引 (§1, §3.1, §4.1, §5.1, {META_BinarySearch})
...
| LOAD-41 | RadixBinaryTree によるファイルオフセット検索 | デコード済みモジュール | `lookup_by_file_offset(offset)` を実行 | 基数表＋有界二分探索（$O(k)$ / $O(\log n)$）により、指定オフセットを包含するデコード済みエンティティが正確に返却される | §4.1「ファイル位置逆引き」, §5.1 `lookup-by-file-offset` |
...
- **ゼロコピー索引化 (LOAD-10〜15)**: ROM直接参照、エクスポート名二分探索、遅延アクセサ。
- **複数モジュール・インポート解決 (LOAD-20〜25)**: インポート解決、型照合、レジストリ上限、LIFOアンロード。
- **容量制約 (LOAD-30〜32)**: 最大数制限およびLEB128ガード。
- **RadixBinaryTree ファイル位置逆引き (LOAD-40〜44)**: デコード済みエンティティ登録、RadixBinaryTree による $O(k)$ 高速検索、関数/データ逆引き、境界テスト。
