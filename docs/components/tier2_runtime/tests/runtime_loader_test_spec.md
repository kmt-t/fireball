...
| TEST-LOAD-25 | アンロードのLIFO制約 | 複数モジュールをロード | ロード順の逆順以外でunload | レジストリからは削除されるが、バンプアロケータのメモリは完全回収されない（ロード逆順でのみ完全回収） | 「アンロード」, unload補足 |
...
| TEST-LOAD-31 | エクスポート数上限 | `FB_CONF_MAX_EXPORTS`（64）超過 | prepare | reject/エラー | runtime_loader.md (Resource Constraints) |
