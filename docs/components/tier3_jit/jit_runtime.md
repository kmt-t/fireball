...
- **JITコード領域 (8KB)**: 4KBページ2枚分の連続領域である。先頭2KBは相対ジャンプ用共通コード等の非エビクション領域とし、残る2KBずつを`Bank 0 (Active)`, `Bank 1 (Warm)`, `Bank 2 (Oldest)`に割り当てる。MPUの`W^X`制御対象であり、共通コード領域はflushやバンクローテーションでも維持する。
