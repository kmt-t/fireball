...
- **JITエントリ表**: 命令オフセットと生成コード位置（`exec_trace`）のペアをソート順で管理する内部配列（プライベートメンバ）。`fireball::flat_map_view` として参照される。詳細メタデータ（サイズ、チェイン先等）は各コード先頭の 16 バイト固定長 `jit_trace_header`（インラインヘッダ）に保持される。
...
5. **局所再チェイニングとアンリンク ($O(k)$)**: キャッシュローテーション発生時、Oldest バンクがパージされ新 Active へ再利用される直前に、そのバンクの「被チェイン逆引きテーブル」に登録されたソースエントリ（$k$ 件）のみを直接参照する。ターゲットが Active/Warm へ昇格済みの場合は昇格先へ再チェイニングし、完全破棄の場合のみインタープリタ復帰スタブにアンパッチする（Warm から Oldest への移行期間中はチェイン実行を維持）。全エントリ走査（全舐め）を行わない。
