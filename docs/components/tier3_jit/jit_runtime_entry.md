...
5. **局所アンリンク ($O(k)$)**: キャッシュローテーション発生時、Oldest バンクがパージされ新 Active へ再利用される直前に、そのバンクの「被チェイン逆引きテーブル」に登録されたソースエントリのみを直接参照し、チェインスロットをインタープリタ復帰スタブにアンパッチする（Warm から Oldest への移行期間中はチェイン実行を維持）。全エントリ走査（全舐め）を行わない。
