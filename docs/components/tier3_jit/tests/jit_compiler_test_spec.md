...
| JITC-56 | SP即値巻き戻しを伴う多段分岐インライン展開 | ラベル脱出に伴うスタック巻き戻しを持つ `BR` / `BR_IF` | トレースコンパイルと実行 | 分岐直前に `sp_offset` への即値加算命令（`ADD.W`）がインライン生成され、複雑なアンワインド機構なしでスタックポインタが即値更新される | 「制御フロー・コール境界のインタープリタ委譲不変条件」 |
