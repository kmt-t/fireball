...
| JITR-26 | Direct-Mapped Folding XOR JIT Cache による O(1) 一発ヒット | トレースがキャッシュに存在 | lookup | 4-bit Folding XOR Hash で 16 スロットテーブルにヒットし、バンク二分探索を行わずに O(1) で即時返却される | `jit_runtime.md` §4.1, `{DirectMappedJIT16}` |
...
| ID | 検証項目 | 前提条件 | 手順 | 期待結果 | 紐付け |
| :--- | :--- | :--- | :--- | :--- | :--- |
| JITR-GOTCHA-01 | キャッシュ常駐性の一次情報源（二重コンパイル防止） | カード状態は未COMPILEDだがキャッシュ内に同一PCが常駐済み | `idle_hook` のキュー処理を実行 | コンパイラは起動されず、カード状態のみ `COMPILED` へ同期される。**実装の勘所**: カードの粗い2-bit状態のみを信じてキューを盲目的に処理すると、同一PCが短期間に複数回キューイングされた場合に二重コンパイルが発生し、メモリを浪費する | `jit_runtime.md` §4.1「キュー処理時のキャッシュ再確認」 |
| JITR-GOTCHA-02 | Oldest昇格時の被チェイン登録移管とダングリング防止 | トレースB（Oldest所属）が別トレースAから被チェインされている状態でBをActiveへ昇格 | Bを昇格後にOldestバンクをローテーション破棄 | 昇格先バンクへAの被チェイン登録が移管されており、Oldest破棄時にAのチェイン先が誤ってアンパッチ（NULL化）されず、直結が維持される。**実装の勘所**: 被チェイン逆引きテーブル（`inbound_sources`）はバンク管理であるため、トレース本体のみを移動して登録の移管を怠ると、バンク消滅時にダングリングジャンプまたは誤ったアンパッチが発生する | `jit_runtime.md` §4.1「局所再チェイニングとアンリンク」 |
| JITR-GOTCHA-03 | LIFO逆順コンパイルによる即時チェイニング最大化 | 連続するブロック（A→B→C）が順にキューへ投入された状態 | `idle_hook` を実行 | 後入れの C、次いで B、最後に A の順（LIFO）でコンパイルされ、先行ブロックがコンパイルされた時点で後続ブロックが既にキャッシュ常駐しているため、即時チェイニングが100%成功する。**実装の勘所**: FIFOでコンパイルすると、先行ブロックコンパイル時に後続がまだ存在しないためチェインが成立せず、余計なインタープリタフォールバックを招く | `jit_runtime.md` §4.1, `{JIT_ReverseCompilationOrder}` |
| JITR-GOTCHA-05 | 3面ローテーション・フラッシュ時の Folding XOR スロット無効化 | rotate() または flush_all() 実行時 | rotate() を実行し直後に旧PCでlookup | 16スロットの高速キャッシュが不可分にクリアされ、古いバンクへのダングリング参照を起こさず、必要に応じて昇格（Promotion）処理が正しく発火する。**実装の勘所**: 高速スロットをクリアしないと、Oldest に遷移したトレースが高速スロット経由で返却されてしまい、昇格処理がスキップされ、次の rotate() で実体が消滅した後にダングリング状態へ陥る | `jit_runtime.md` §4.1 手順7 |
| JITR-GOTCHA-06 | JIT脱出後の制御フレーム整合（フレームスタック回収） | ループがif文の中に入れ子になっており（例: ループの中でif文が成立した時だけ別のループを回す構造）、内側ループの脱出条件ブロックがコンパイル済み | 内側ループ・外側ループの双方が複数回実行されるまで駆動を継続する | 外側ループ自身の分岐がインタープリタで正しく外側ループの先頭へ解決され、結果値が期待通り一致する。**実装の勘所**: JITによる脱出は、ブロック・ループ・ifの開始や終了に対応するインタープリタ側のフレーム操作を一切経由しない。そのため、以前インタープリタが直接その構文を実行していた際に積まれた制御フレームが、JIT側でその構文を抜けた後も回収されずに残ってしまうことがある。残ったままだと、後続の深さ相対な分岐命令が誤った階層を対象に解決してしまう。次に実行するブロックが本来持つべきフレーム深さまで、実行直前に必ず制御フレームを巻き戻すことでこれを防ぐ | `jit_runtime.md` §4.3, `runtime_interpreter.md` §3.3「制御フレーム整合性」 |
| JITR-GOTCHA-07 | 短小ブロック判定の符号（後方分岐ブロックの誤除外） | ループ本体の末尾が、自分自身の先頭よりも前のアドレスへ戻る後方分岐になっている | 短小ブロックの足切り判定を経て駆動を反復実行する | 後方分岐を持つブロックも足切り判定を正しく通過し、十分な命令数を持てばコンパイル対象になる。**実装の勘所**: 足切り判定を「後続アドレスと自分自身の先頭との差分」で代用すると、後方分岐ブロックでは差分が常に負値になり「短すぎる」と誤判定されて、記録・コンパイルのいずれの対象からも永久に外れてしまう。関数内で最も実行頻度の高いブロック（ループ本体そのもの）が二度とコンパイルされないまま、常にインタープリタ実行に取り残される | `jit_runtime.md` §4.3 |
| JITR-GOTCHA-08 | RETURN終端ブロックの脱出は定数時間の番兵値で、命令の読み直しは不可 | コンパイル済みブロックの終端が関数からの復帰（進む先のアドレスが存在しない） | 当該トレースを実行し、結果を検証する | コード全体の長さを指す番兵値（定数時間で求まる一つの数値）で解決され、後続のインタープリタが関数終了・呼び出し元への復帰処理を正しく行う。**実装の勘所**: 命令列を読み直して終端位置を割り出す方法で解決すると、`{DirectBytecodeExecution}`（`runtime_interpreter_test_spec.md` INTP-GOTCHA-05）が禁じる、実行時の命令の事前デコードや命令オブジェクトの生成が発生してしまう | `jit_runtime.md` §4.3, `{DirectBytecodeExecution}` |
