...
| MEM-10c | `route_message`のキュー満杯時ロールバックとowner_idの復元 | 送信先チャネルが満杯 | `ipc.send`を実行 | `ERR_QUEUE_FULL`を返し、対応するvMMIO PTEの`owner_id`が送信元タスクIDへ復元される（`FLIGHT`のまま放置されない） | §7 手順5（本セッションでの訂正後）, ipc_router.md §4.1 Rollback |
