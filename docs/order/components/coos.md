...
- **協調的マルチタスク**
  ...
  - 導出元: `{CooperativeMultitasking}` ([`docs/gen/requires/list.md`](docs/gen/requires/list.md))
- **CSPベースの同期**
...
- **タスクスケジューリング**
  ...
  - 導出元: `{TaskScheduling}` ([`docs/gen/requires/list.md`](docs/gen/requires/list.md))
- **CSPチャネル通信**
  - 概要: 型安全なメッセージパッシング。1エントリのバッファを持つブロッキング通信。
  - 導出元: `{CSPChannel}` ([`docs/gen/architecture/overview.md`](docs/gen/architecture/overview.md))
- **メモリ隔離管理**
  - 概要: タスクごとの独立したヒープ領域の提供。タスク終了時に自動的に回収される。
  - 導出元: `{MemoryIsolation}` ([`docs/gen/requires/list.md`](docs/gen/requires/list.md))
- **所有権管理**
...
- **C++20コルーチンの活用**
  ...
  - 導出元: `{UseCpp20Coroutine}` ([`docs/gen/architecture/overview.md`](docs/gen/architecture/overview.md))
- **独立ヒープ**
  - 概要: `dlmalloc`をベースに、タスクごとに隔離されたメモリプールを割り当てる。
  - 導出元: `{IndependentHeap}` ([`docs/gen/architecture/overview.md`](docs/gen/architecture/overview.md))
...
- **低オーバーヘッドなコンテキストスイッチ**
  ...
  - 導出元: `{LowOverheadSwitch}` ([`docs/gen/architecture/overview.md`](docs/gen/architecture/overview.md))

### メモリ制約と方策
- **厳密なメモリ制限**
  - 概要: タスク起動時に指定されたサイズ以上のメモリ確保を禁止する。枯渇時はタスクを異常終了させ、システム全体への波及を防ぐ。
  - 導出元: `{StrictMemoryLimit}` ([`docs/gen/architecture/overview.md`](docs/gen/architecture/overview.md))

### 安全性制約と方策
- **データ競合の排除**
  - 概要: CSPモデルと所有権管理により、ミューテックスなしで安全なデータ共有を実現する。
  - 導出元: `{EliminateDataRace}` ([`docs/gen/requires/list.md`](docs/gen/requires/list.md))
- **障害隔離**
  - 概要: 特定タスクのヒープ枯渇やクラッシュが他タスクやカーネルに影響しないよう、リソースを完全に分離する。
  - 導出元: `{FaultIsolation}` ([`docs/gen/architecture/overview.md`](docs/gen/architecture/overview.md))
- **割り込みハンドラの安全性**
