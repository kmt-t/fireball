...
- Tier0サービス・ゲストモジュールの障害時は、vSoCがリカバリ用イベントハンドラを介して対応する。

| パーティション名 | 目的 | 最小サイズ | メモリ確保失敗時の影響 |
|---|---|---|---|
| ネイティブヒープ | スケジューラ, CSP, 共有メモリ等 | 4.0KB | アボート |
| vSoCヒープ | インタープリタ, コンテキスト等 | 2.0KB | ハイパーバイザ終了 |
| サブシステム・Tier1サービスヒープ | IPCルータ, HAL等 | 2.0KB | ハイパーバイザ終了 |
| wasmリニアメモリ | ゲストアプリケーション | 8KB | ゲストのみ終了 |
