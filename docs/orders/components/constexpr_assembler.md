...
| 項目名 | 機能と役割 | 備考（制約、型など） |
| :--- | :--- | :--- |
| 命令階層 | RISC-Vの基本命令セットにおける大分類（OP-IMM等）。 | `field.opcode` |
| 対象レジスタ | 演算結果が書き込まれるデスティネーション。 | `field.rd` |
| 詳細機能 | 命令内のサブ操作を定義する。 | `field.funct3` |
| 基点レジスタ | 演算またはアドレス計算の基点となるソース。 | `field.rs1` |
| 組込即値 | 12ビットの符号付き即値データ。 | `field.imm11_0` |
...
| 項目名 | 機能と役割 | 備考（制約、型など） |
| :--- | :--- | :--- |
| 基本命令部 | Thumb-2命令の主要なビットパターン。 | 16 bit |
| 結果レジスタ | 命令の出力先。 | `field.rd` |
| 演算レジスタ | 命令の入力元。 | `field.rn` |
| 合成即値 | 複数のフィールドを組み合わせて作成される数値データ。 | `field.i:field.imm3:field.imm8` |
| 更新フラグ | 条件フラグ（APSR）を更新するかどうかを指定。 | `field.S` |
...
| 項目名 | 機能と役割 | 備考（制約、型など） |
| :--- | :--- | :--- |
| 基本命令コード | レジスタ番号を含む命令の先頭バイト。 | 1 byte |
| 32bit即値 | 命令の後に続く符号付き整数データ。パッチ対象。 | 4 bytes |
