...
| :--- | :--- | :--- |
| `id` | デバイスを一意に識別するためのシステム全体で重複しない識別子。 | `device_id` 型 |
| `name` | デバイスの種類や役割を示す人間が読める名前。 | `char[16]` |
| `type` | デバイスの入出力特性（ブロック単位、ストリーム単位、等）。 | 列挙型 |
| `block_size` | このデバイスにおける最小の転送/処理単位。 | バイト数 |
...
| 構成項目 | 機能と役割 | 備考（制約、型など） |
| :--- | :--- | :--- |
| `max_devices` | システムに登録可能な最大デバイス数。静的なレジストリサイズを決定する。 | 1-255 |
| `max_buffers` | 通信に使用するバッファプールの最大数。 | 1-255 |
