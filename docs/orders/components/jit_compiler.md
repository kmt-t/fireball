...
| `code_offset` | キャッシュ（Active領域）の先頭からの相対位置。 `code_align_shift` 分右シフトされた値。 | 16bit値 |
...
4. **オンデマンド・キューイング (Hot Card Fallback)**:
    - Active/Old両方でミスし、かつ Bitmap 状態が `COMPILED (3)` の場合、対象のPCを `Compile Queue` へプッシュする。実際のコンパイルは次回のバッチ処理（Idle時等）で行われる。
5. **結果の返却**:
    - ヒット（または昇格成功）時はネイティブコードのアドレスを返す。
    - いずれの領域でもミスした場合は、(たとえ `COMPILED` カードであっても) NULL を返し、インタープリタ実行を継続する。
