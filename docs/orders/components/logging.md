...
- **`Logger` (Class)**: ログの収集、バッファリング、および物理出力への転送を一括して管理する主要クラス。
- **`logging_config` (View)**: バッファサイズやデフォルトログレベルなどの不変な構成情報。
- **`log_entry` (Internal)**: リングバッファに格納される単一のログデータ構造（辞書オフセット + 引数）。
...
| 項目名 | 機能と役割 | 備考（制約、型など） |
| :--- | :--- | :--- |
| 出力トランスポート | 物理的なログ出力（UART等）を担うHALへの参照。 | `hal_transport*` |
| 循環バッファ | ログデータを一時的に保持する領域。 | 固定長配列等 |
| 書き込み/読み出し索引 | バッファの現在の状態を示すポインタ。 | アトミック操作推奨 |
| 出力閾値 | 現在出力対象としている最小のログレベル。 | `log_level` |
...
| 引数と役割 | `level`: 重要度, `offset`: 辞書位置, `args`: 数値パラメータ |
