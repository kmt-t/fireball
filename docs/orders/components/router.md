...
| :--- | :--- | :--- |
| `type_scope` | 上位3ビットでスコープ、下位5ビットでデータの型（定数、ハンドル、ID等）を定義する。 | 8bit値 |
| `key` | スコープ（Functional/Dictionary）に応じた、データの意味を識別するためのキー。 | 24bit値 |
| `value` | 実際のデータ本体、あるいはリソースを指し示すハンドルや即値。 | 32bit値 |
...
| 構成項目 | 機能と役割 | 備考（制約、型など） |
| :--- | :--- | :--- |
| `pairs` | 通信内容を構成するKey-Valueペアの固定長配列。 | 8個固定 |
...
| 構成項目 | 機能と役割 | 備考（制約、型など） |
| :--- | :--- | :--- |
| `data_ptr` | 元の `kv_pair` 配列へのアクセスポインタ。 | 不変ポインタ |
| `indices` | キーに基づいてソートされた要素のインデックスを保持する。二分探索に使用。 | 8bit×8エントリ |
...
| 構成項目 | 機能と役割 | 備考（制約、型など） |
| :--- | :--- | :--- |
| `uri` | サービスを一意に特定するためのURI文字列。検索の主キーとなる。 | `const char*` |
| `role` | サービスに割り当てられたセキュリティロール。アクセス制御に使用。 | ロール定義型 |
| `channel_id` | サービスがメッセージを待機している対応チャネルの識別子。 | `channel_id` 型 |
...
| 引数と役割 | `uri`: サービスURI, `role`: アクセス権限, `channel`: 通信チャネル。 |
| 期待する結果 | 正常：サービスが登録され、他のタスクから検索可能になる。 |
| 事前条件 | レジストリに空きがあること。URIが重複していないこと。 |
| 事後条件 | レジストリがURI順に維持され、高速検索が保証される。 |
| 不変条件 | システムのロールマトリックスと矛盾しないこと。 |
| エラー時の挙動 | 重複や容量不足の場合はエラーを返す。 |
| 補足 | 通常、システムの初期化フェーズで各コンポーネントが呼び出す。 |

#### サービスの名称解決 (Lookup)
| 項目 | 内容 |
| :--- | :--- |
| 機能概要 | 指定されたURIに対応する通信用チャネルIDを取得する。同時に送信側の権限チェックを行う。 |
| 引数と役割 | `uri`: 検索対象のサービスURI。 |
| 期待する結果 | 正常：通信可能なチャネルID。異常：権限不足または未登録。 |
| 事前条件 | なし。 |
| 事後条件 | なし。 |
| 不変条件 | 検索過程でレジストリが変更されないこと。 |
...
| 引数と役割 | `channel`: 送信先ID, `msg`: 送信メッセージ。 |
| 期待する結果 | 正常：メッセージが相手タスクに届き、所有権が安全に移譲される。 |
| 事前条件 | チャネルが有効であること。 |
| 事後条件 | メッセージ内のハンドルが含まれる場合、送信側からのアクセス権が消失する。 |
| 不変条件 | メッセージの内容が改ざんされないこと。 |
| エラー時の挙動 | 送信失敗時はエラーを返し、所有権の移譲を中止する。 |
| 補足 | カーネル内で `co_csp` と連携して動作する。 |
...
- [ ] 内部ブロック図（静日）とシーケンス/状態遷移図（動的）がセットで定義されているか
