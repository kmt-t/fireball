...
| :--- | :--- | :--- |
| `name` | サービスを識別するための共通名称。 | `const char*` |
| `tier` | 隔離レベルの定義。0 はゲストへの直接リンク（高速）、1 は独立タスクによるIPC経由。 | 8bit値 |
| `uri` | IPCルータで公開される、サービスへのアクセス基点。 | `const char*` |
...
| 構成項目 | 機能と役割 | 備考（制約、型など） |
| :--- | :--- | :--- |
| `guest_id` | 設定が適用されるWASMゲストの識別子。 | 32bitID |
| `load_list` | ゲストの起動時に自動的に解決・連結されるサービスのURIリスト。 | 文字列ポインタ配列 |
