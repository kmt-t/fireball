...
<<<<<<< HEAD
  - **ステップ実行の制御**: 実行コンテキストに `step_mode` フラグを保持する。デバッガ有効時のハンドラは、1命令実行ごとにこのフラグをチェックし、真であればインタープリタのループを抜けてデバッガに制御を戻す。
=======
...
<<<<<<< HEAD
- サービスによる命令フックが有効な場合、追加コードを埋め込む。ただし、デバッガがアクティブな期間は、実装規模抑制のためJIT実行を停止し、インタープリタ実行にフォールバックする。
=======
