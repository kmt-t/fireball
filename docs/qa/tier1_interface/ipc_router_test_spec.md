...
| TEST-IPCR-12 | 相手未到達時のIn-flight待機 | メッセージがRevokeされIN_FLIGHTになり、相手タスクを到達させない | `csp_handoff_model.py`とチャネル状態を確認 | 送信側はIN_FLIGHTで待機し続けてもよい。相手タスク到達後はGrantへ進むが、CSP単独の全タスク公平性や有界応答時間を要件にしない | `ipc_router.md`「In-flight待機の安全性」 |
