...
ページ単位権限分離、`shared-block`と物理 vMMIO PTE/TLB との連動、Cortex-M33 PMSAv8 MPUリージョン配分とJIT W^X切替プロトコルという、`system_allocator`/`shm_allocator` の物理実装レベルの振る舞いを検証する。契約レベルの公開API振る舞い（TEST-MEM-01〜13）は [`system_memory_test_spec.md`](docs/qa/tier1_interface/system_memory_test_spec.md) の責務とする。
