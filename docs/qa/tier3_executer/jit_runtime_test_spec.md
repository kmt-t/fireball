...
| TEST-JITR-65 | ctypes dispatch snapshotの世代キャッシュとhotspot profiling切替 | 未コンパイル候補blockと一つの常駐traceを持つJIT manager | `native_dispatch_state()`を反復取得し、descriptorフィールド、trace挿入、hotspot profiling切替後のsnapshotを確認する | 状態不変中は同じctypes配列を再利用する。キャッシュ世代変更後はtrace表を再構築する。C++ dispatcherは固定配列bufferを直接読み、hotspot profiling無効時は候補PC数を0にして常駐trace実行と分岐handlerの意味を保つ | `jit_runtime.md`「トレース実行時の分岐解決とインタープリタ復帰」、pysim `test_jitr_native_dispatch_snapshot_is_cached_and_hotspot_collection_is_configurable` |
