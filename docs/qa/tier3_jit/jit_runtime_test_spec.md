...
| テストケースID | 検証項目 | 前提条件 | 手順 | 期待結果 | 紐付け |
| :--- | :--- | :--- | :--- | :--- | :--- |
| TEST-JITR-44 | JIT終了ブロックからInterpreterのreturnハンドラへ復帰 | JITトレースが`return`直前で終了する関数 | トレース後のInterpreter PCと共有スタックを確認する | JITはC/AAPCS戻り値レジスタを使わず共有OperandStackへ結果を書き、raw bytecode上の`return`命令位置へ戻る。Interpreterのreturnハンドラだけがsentinelを生成する | `runtime_interpreter.md` 関数復帰の番兵, pysim `test_jitr_terminal_trace_returns_to_interpreter_return_handler` |
| TEST-JITR-45 | JIT実行calleeの戻り値をcallerへ引き継ぐ | callerが反復してcalleeを呼び、calleeのブロックがJITコンパイル済み | `RuntimeEngine.run()`でcallerを実行 | calleeの結果は共有OperandStackに残り、InterpreterがRETURN sentinelを消費してcalleeの`CallFrame`をpopし、callerがその結果を利用する | `runtime_interpreter.md` 関数復帰の番兵, pysim `test_jitr_nested_wasm_call_keeps_callee_result_on_shared_operand_stack` |
...
| TEST-JITR-47 | import/host callをInterpreter境界で実行 | ゲスト関数がWASM importを呼び、戻り値を後続命令で使う | 呼び出しブロックのコンパイルとRuntimeEngine実行を確認する | callを含むトレースはコンパイルされず、Interpreterがhost関数を呼ぶ。戻り値は共有OperandStackに積まれ、callerの後続命令が消費する | `runtime_interpreter.md` 関数呼び出し境界, pysim `test_jitr_host_import_stays_on_interpreter_runtime_boundary` |
...
| GOTCHA-JITR-08 | return直前のJIT終了とInterpreter復帰 | `return`を含むWASM関数 | トレース実行とcallee復帰を確認 | JITは`return`や`RETURN sentinel`を生成せず、終了エピローグで共有OperandStack／実行コンテキストを同期してInterpreterへ戻る。Interpreterがcalleeの`CallFrame`をpopし、ネスト時はcall helperがsentinelを消費する | `jit_runtime.md`, `runtime_interpreter.md` |
