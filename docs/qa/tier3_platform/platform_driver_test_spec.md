...
物理割り込みのpush経路、GPIO直接ストアの高速パス、HALバッファプール(vMMIO/DYNAMIC)への物理マッピング、RSPトランスポートの物理エンコード/デコードを検証する。契約レベルの振る舞い（IPCルータ経由アクセス、hal-buf-id契約等）は [`hal_dispatch_test_spec.md`](docs/qa/tier2_runtime/hal_dispatch_test_spec.md) の責務とする。
