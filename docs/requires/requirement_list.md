...
| `{ThreadedInterpreter}` | テーブルディスパッチによる高速な命令実行と、環境ポインタによる階層アクセスを実現する。 | 高 | テスト |
...
| `{RestrictedPhysicalAccess}` | 物理リソースへのアクセスを許可テーブルで厳格に制限する。 | 高 | テスト |
...
| `{RecoveryStrategy}` | エラーコードの代わりに推奨されるリカバリー動作（Retry/Panic等）を返す。 | 中 | レビュー |
...
| `{CSP_Handoff}` | 送受信時に相手が待機状態であれば、スケジューラを介さず直ちにスイッチする。 | 高 | テスト |
| `{PeriodicTask}` | システムティックまたはアイドルループを利用した定期実行タスクをサポートする。 | 中 | テスト |
| `{IdleDetection}` | システムのアイドル状態を検知し、バックグラウンド処理（GC/ログ出力）を実行する。 | 中 | テスト |
| `{DirectContextSwitch}` | スケジューラを介さない、超低レイテンシなタスク切り替え。 | 高 | ベンチマーク |
...
| `{NoStdVector}` | 動的な `std::vector` の使用を禁止し、固定長またはカスタムコンテナを使用する。 | 高 | レビュー |
...
| `{ConsolidatedHeap}` | システムとタスクのヒープを統合し、メモリ効率を最大化する。 | 高 | 計測 |
| `{StrictMemoryLimit}` | 厳格なメモリ割り当て制限（例：20KB/64KB）を適用する。 | 高 | テスト |
| `{IndependentHeap}` | 各セキュリティドメインやコンポーネントに独立したヒープ領域を割り当て、障害を隔離する。 | 高 | テスト |
| `{Policy_Memory}` | ヒープの使用を原則禁止し、静的またはスタック割り当てを優先する組み込みポリシー。 | 高 | プロセス監査 |
...
| `{FaultIsolation}` | メモリパーティションにより、コンポーネント間の障害伝播を防止する。 | 高 | テスト |
| `{3TierSeparation}` | アーキテクチャ、サブシステム、実装の3層に厳密に分離し、依存関係を管理する。 | 高 | レビュー |
| `{ComponentHarness}` | コンポーネントの依存関係をハーネス構造体に集約し、テスト時の注入を容易にする。 | 高 | レビュー |
| `{StaticScalability}` | リソース上限をコンパイル時定数で決定し、動的拡張のオーバーヘッドを排除する。 | 高 | レビュー |
| `{WIT_First}` | WebAssembly Interface Types (WIT) はシステムインターフェースの唯一の真実在であり、設計は常にここから開始する。 | 高 | レビュー |
| `{Risk_Tiering}` | リスクベースの設計階層化。重要度や不確実性に応じて検証レベルを調整する。 | 高 | レビュー |
| `{Type_Vocabulary}` | 仕様と実装を正確に紐付けるための厳格な型エイリアス定義と語彙セット。 | 高 | レビュー |
| `{ZeroCostAbstraction}` | 抽象化のコストを実行時に支払わない。C++非仮想インターフェース等による最適化。 | 高 | レビュー |
