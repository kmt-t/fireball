...
| Opcode | 命令名 | スタック遷移 | インタープリタ実装 | JIT Stencil 提供 | 物理動作・備考 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `0x20` | `local.get` | `[] -> [t]` | ローカル配列 `[local_base + idx]` をロード | あり (Direct LDR / Mov) | `LDR r4, [r3, #offset]` またはレジスタ転送 |
| `0x21` | `local.set` | `[t] -> []` | ローカル配列 `[local_base + idx]` へストア | あり (Direct STR / Mov) | `STR r4, [r3, #offset]` またはレジスタ代入 |
| `0x22` | `local.tee` | `[t] -> [t]` | ローカルへ保存しつつスタックに残す | あり (STR & Keep) | `STR r4, [r3, #offset]` (TOS維持) |
...
| Opcode | 命令名 | スタック遷移 | インタープリタ実装 | JIT Stencil 提供 | 物理動作・備考 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `0x28` | `i32.load` | `[i32] -> [i32]` | アドレス境界マスク $\to$ 32-bit ロード | あり (LDR.W) | `LDR r4, [r3, r4]` (`r3=mem_base`) |
| `0x29` | `i64.load` | `[i32] -> [i64]` | アドレス境界マスク $\to$ 64-bit ロード | あり (LDRD) | `LDRD r4, r5, [r3, r4]` |
| `0x2A` | `f32.load` | `[i32] -> [f32]` | アドレス境界マスク $\to$ 単精度ロード | あり (VLDR.32) | `VLDR s0, [r3, r4]` (FPU搭載時) |
| `0x2B` | `f64.load` | `[i32] -> [f64]` | アドレス境界マスク $\to$ 倍精度ロード | あり (VLDR.64) | `VLDR d0, [r3, r4]` (FPv5搭載時) |
| `0x2C` | `i32.load8_s`| `[i32] -> [i32]` | 符号拡張 8-bit ロード | あり (LDRSB) | `LDRSB r4, [r3, r4]` |
| `0x2D` | `i32.load8_u`| `[i32] -> [i32]` | ゼロ拡張 8-bit ロード | あり (LDRB) | `LDRB r4, [r3, r4]` |
| `0x2E` | `i32.load16_s`| `[i32] -> [i32]`| 符号拡張 16-bit ロード | あり (LDRSH) | `LDRSH r4, [r3, r4]` |
| `0x2F` | `i32.load16_u`| `[i32] -> [i32]`| ゼロ拡張 16-bit ロード | あり (LDRH) | `LDRH r4, [r3, r4]` |
| `0x36` | `i32.store` | `[i32, i32] -> []` | 32-bit メモリストア | あり (STR.W) | `STR r5, [r3, r4]` |
| `0x37` | `i64.store` | `[i32, i64] -> []` | 64-bit メモリストア | あり (STRD) | `STRD r5, r6, [r3, r4]` |
| `0x38` | `f32.store` | `[i32, f32] -> []` | 単精度メモリストア | あり (VSTR.32) | `VSTR s0, [r3, r4]` |
| `0x39` | `f64.store` | `[i32, f64] -> []` | 倍精度メモリストア | あり (VSTR.64) | `VSTR d0, [r3, r4]` |
| `0x3A` | `i32.store8` | `[i32, i32] -> []` | 8-bit メモリストア | あり (STRB) | `STRB r5, [r3, r4]` |
| `0x3B` | `i32.store16`| `[i32, i32] -> []` | 16-bit メモリストア | あり (STRH) | `STRH r5, [r3, r4]` |
