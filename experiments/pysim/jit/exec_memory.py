...
if IS_WINDOWS:
    ...
    _libc.mmap.argtypes = [ctypes.c_void_p, ctypes.c_size_t, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int64]
...
class ExecutableBuffer:
    ...
    def __init__(self, size: int):
        ...
        if IS_WINDOWS:
            ...
            addr = _libc.mmap(None, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)
    ...
    def assert_no_rwx(self) -> None:
        ...
        if IS_WINDOWS:
            ...
            assert self.current_protection != (PROT_READ | PROT_WRITE | PROT_EXEC), "Invariant violation: Buffer is in RWX state"

            assert self.current_protection in (PROT_READ | PROT_WRITE, PROT_READ | PROT_EXEC, PROT_READ)
