...
def run_i32(body_stencils_with_patches: list[tuple[st.Stencil, dict]],


            locals_values: list[int] | None = None,


            memory: bytearray | None = None) -> int:
...
def run_i32_checked(body_stencils_with_patches: list[tuple[st.Stencil, dict]],
...
def test_local_tee_writes_the_slot_but_also_leaves_the_value_on_the_stack():
    ...
    code2 = [const_(77), (st.LOCAL_TEE, {"disp": 0}), (st.DROP, {}), (st.LOCAL_GET, {"disp": 0})]
...
def test_local_get_set_tee_at_a_nonzero_locals_array_offset():
    ...
    code = [const_(555), (st.LOCAL_SET, {"disp": 2 * 8}), (st.LOCAL_GET, {"disp": 2 * 8})]
...
def test_i32_div_and_rem_signed_and_unsigned():
    ...
    assert run_i32(push_two(_to_i32(0xFFFFFFF0), 2) + [(st.I32_DIV_U, {})]) == _to_i32(0xFFFFFFF0 // 2)


    assert run_i32(push_two(_to_i32(0xFFFFFFF0), 3) + [(st.I32_REM_U, {})]) == _to_i32(0xFFFFFFF0 % 3)
...
def test_i32_shifts_mask_the_count_to_5_bits_like_wasm_requires():
    ...
    assert run_i32(push_two(_to_i32(0x80000000), 1) + [(st.I32_SHR_U, {})]) == 0x40000000
...
def test_i32_store_then_i32_load_round_trip():
    ...
    code = [
        ...
        const_(8), const_(0x11223344), (st.I32_STORE, {"disp": 0}),


        const_(8), (st.I32_LOAD, {"disp": 0}),
...
def test_i32_load_store_honor_the_memarg_static_offset():
    ...
    code = [const_(4), const_(99), (st.I32_STORE, {"disp": 12}), const_(0)]  # writes at addr 4+12=16
...
def test_in_bounds_access_does_not_trap():
    ...
    code = [const_(12), const_(0x2A), (st.I32_STORE, {"disp": 0, "max_addr": 12}), const_(0)]
...
def test_out_of_bounds_load_traps():
    ...
    code = [const_(13), (st.I32_LOAD, {"disp": 0, "max_addr": 12})]   # addr=13 > max_addr=12
...
def test_i32_store8_and_store16_write_only_their_width():
    ...
    run_i32([const_(0), const_(0x1234), (st.I32_STORE8, {"disp": 0}), const_(0)], memory=memory)
    ...
    run_i32([const_(0), const_(0x12345678), (st.I32_STORE16, {"disp": 0}), const_(0)], memory=memory2)
