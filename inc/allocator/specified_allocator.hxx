 ...
 *
 * This allocator wraps dlmalloc's mspace interface to provide flexible
 * allocation and deallocation within a fixed-size arena. Unlike bump_allocator,
 * it supports arbitrary allocation patterns with full deallocation capability,
 * making it suitable for heap partitions with dynamic memory requirements
 * (e.g., subsystem heap, service heap). The allocator manages fragmentation
 * through dlmalloc's internal strategies and provides O(log n)
 * allocation/deallocation time complexity.
 *
 * Template Parameters:
...
public:
  ...
  void *do_allocate(std::size_t bytes, std::size_t alignment) override {
    return mspace_ == nullptr ? nullptr
                              : mspace_memalign(mspace_, alignment, bytes);
  }
