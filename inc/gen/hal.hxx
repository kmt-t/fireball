...
public:
   ...
   * Initializes the HAL.
   */
  operation_result initialize() noexcept;
   ...
   */
  std::expected<uint32_t, recovery_strategy> read(device_id id, shm_id dest) noexcept;
   ...
   * @pre: src != 0
   */
  operation_result write(device_id id, shm_id src) noexcept;
   ...
   */
  operation_result ioctl(device_id id, uint32_t cmd, message params) noexcept;

  /**
   * Gets information about a device.
   */
  std::expected<device_info, bool> query_device(device_id id) noexcept;

  /**
   * Gets information about a buffer.
   */
  std::expected<buffer_info, bool> query_buffer(shm_id id) noexcept;
   ...
   */
  std::expected<shm_id, recovery_strategy> acquire_buffer(uint32_t size) noexcept;
   ...
   */
  void release_buffer(shm_id id) noexcept;
...
public:
   ...
   */
  std::expected<bool, recovery_strategy> get_pin(uint32_t pin) noexcept;
...
public:
   ...
   */
  shm_id subscribe_duration(uint64_t nanos) noexcept;
...
public:
   ...
   */
  operation_result transfer(shm_id tx_buffer, shm_id rx_buffer) noexcept;
...
public:
   ...
   */
  operation_result set_response(shm_id buffer) noexcept;

  /**
   * Gets the received buffer from the master write.
   */
  std::expected<shm_id, recovery_strategy> get_received() noexcept;

  /**
   * Subscribes to bus events.
   */
  shm_id subscribe() noexcept;
...
public:
   ...
   */
  operation_result write(shm_id buffer) noexcept;
...
public:
   ...
   */
  operation_result read(shm_id buffer) noexcept;
...
public:
  ...
  std::expected<shm_id, recovery_strategy> get_parsed_command() noexcept;
