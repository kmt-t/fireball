...
- **評価セクション総数**: 782
- **形式検証 (pyModelChecking) 推奨セクション**: 32
- **LLM 意味監査 推奨セクション**: 5
...
| `components/tier3_jit/jit_engine_copy_patch.md` | JIT トレース物理メモリレイアウト (JIT Trace Header & Binary Layout) | 3 | 2/5 | 2/5 | `Static` | - | Independent heuristic evaluation for 'JIT トレース物理メモリレイアウト (JIT Trace Header & Binary Layout)'. |
...
| `architecture/master_physical_design.md` | Fireball マスター物理設計仕様書 (Master Physical Design) {VERIFY_LLM} | meta | 2/5 | 2/5 | `Static` | - | Independent heuristic evaluation for 'Fireball マスター物理設計仕様書 (Master Physical Design) {VERIFY_LLM}'. |
...
| `components/tier2_runtime/runtime_loader.md` | WASMローダ コンポーネント設計書 {VERIFY_CONCEPT} | 2 | 2/5 | 2/5 | `Static` | - | Independent heuristic evaluation for 'WASMローダ コンポーネント設計書 {VERIFY_CONCEPT}'. |
...
| `components/tier3_jit/jit_assembler_constexpr.md` | コンポーネント設計：constexpr Assembler {VERIFY_BENCHMARK} | 3 | 2/5 | 2/5 | `Static` | - | Independent heuristic evaluation for 'コンポーネント設計：constexpr Assembler {VERIFY_BENCHMARK}'. |
...
| `architecture/master_physical_design.md` | 3.2 メモリ常駐構造体の物理バイトオフセット (Physical Byte Offsets of Memory-Resident Structures) | meta | 2/5 | 2/5 | `Static` | - | Independent heuristic evaluation for '3.2 メモリ常駐構造体の物理バイトオフセット (Physical Byte Offsets of Memory-Resident Structures)'. |
...
| `specs/jit_stencil_catalog.md` | `local_param` (`local_base`) ピン留めバリアント（直交軸） | None | 2/5 | 2/5 | `Static` | - | Independent heuristic evaluation for '`local_param` (`local_base`) ピン留めバリアント（直交軸）'. |
