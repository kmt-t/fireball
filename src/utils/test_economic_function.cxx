...
int main() {
    ...
    fireball::utils::economic_function<int(int)> f1 = [](int x) { return x * 2; };
    assert(f1(5) == 10);
    std::cout << "Test 1 passed: Capture-less lambda" << std::endl;

    // 2. Small capture (fits in 16 bytes)
    int a = 10;
    fireball::utils::economic_function<int(int)> f2 = [a](int x) { return x + a; };
    assert(f2(5) == 15);
    std::cout << "Test 2 passed: Small capture" << std::endl;

    // 3. Move construction
    fireball::utils::economic_function<int(int)> f3 = std::move(f2);
    assert(f3(10) == 20);
    assert(!f2);
