...
組み合わせ検証では、単なる同一性ではなく、ひとつのテーマについて役割分担・依存方向・制約が一貫した物語になっているかを確認する。
...
- **特殊キーワードの分類とフィルタ仕様**:
  - **メタキーワード (`META_` プレフィックス)**: 親子文書ペア解決や整合性チェックリストから常に除外され、主に検証プロンプトのコンテキスト（開発規約ポリシーなど）として利用される。
  - **グローバルキーワード (`GLOBAL_` プレフィックス)**: 個別モジュール単体チェック（`S-TRACE-ALIGN`）では検証対象とされるが、ファイル間整合性チェックリスト（`S-ARCH-CHECKLIST`）からは重複・ノイズ削減のために除外される。
...
#### 検査結果（check_result） `{META_AI_Native_Dev}`
...
#### LLMクライアント構成（client_config） `{META_AI_Native_Dev}`
LLM APIへの接続設定を管理する。
...
- **方策**: `{META_AI_Native_Dev}` 検証が不要なドキュメント（前回コミットから差分がないもの）を自動で判別する仕組み（将来拡張）に対応可能なファイル単位のテスト粒度を維持する。
...
全体ドキュメント検証の課題として、Tier間の階層検証時に参照ドキュメントが散在するため、LLMが完全なコンテキストを持たないという問題がある。これを解決するため、セクション単位でドキュメントを細分化し、マトリクス形式で統合してからレビューするシステムを導入する。 `{META_AI_Native_Dev}`
...
#### セクション（Section） `{META_AI_Native_Dev}`
...
1. **キーワード共有優先**: 親セクションと子セクション内に共通の `{Keyword}` が存在する場合、それらを対応ペアとする。信頼度は共有キーワード数の比率で算出。ただし、メタキーワード（`META_` プレフィックス）およびグローバルキーワード（`GLOBAL_` プレフィックス）はマッチング対象から除外される。
...
| チェック項目 | 検証内容 |
| :--- | :--- |
| テーマ・責務一貫性 | 同じテーマを異なる粒度で説明していても、役割分担・依存方向・制約が一貫しているか。 |
| API/インターフェース整合性 | 引数・戻り値の型、説明、シグネチャが一致しているか。表現差だけで FAIL にしない。 |
...
| :--- | :--- | :--- |
| Fireball 整合性チェッカー | `./tools/run_consistency_check.sh` | プロンプト送信・解析ロジック、および機械的チェックの親実装として。 |
