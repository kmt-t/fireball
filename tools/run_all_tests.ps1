...
if ($assess) {
    ...
    & uv @assessArgs
    if ($LASTEXITCODE -ne 0) {
        Write-Host "✖ Content Complexity & Risk Assessment: FAILED" -ForegroundColor Red
        exit 1
    }
    Write-Host "✔ Content Complexity & Risk Assessment: PASSED" -ForegroundColor Green
} else {
    Write-Host "`n>>> [Phase 2/3] Skipping Risk Assessment (Use -assess to enable)" -ForegroundColor DarkGray
}
...
if ($llm) {
    ...
    Write-Host "`n>>> [Phase 3/3] Skipping LLM as a Judge (Use -llm to enable)" -ForegroundColor DarkGray
}
